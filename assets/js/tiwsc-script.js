jQuery(document).ready(function ($) {
  ...
  window.addedMap = addedMap // Expose for debugging
  ...
  function updateForForm ($form) {
    ...
    console.log('[TIWSC] updateForForm:', {
      pid,
      currentKey,
      isAdded,
    ...
    const $mainButtons = $(
      `.tiwsc-variable-sample-main-button[data-product-id="${pid}"]`
    )
    ...
    $mainButtons.each(function () {
      ...
      if (isAdded) {
        ...
        const originalLabel =
          $label.data('original-label') || 'Gratis Kleurstaal'
  ...
  $(document).on('click', '.cfvsw-swatches-option', function () {
    ...
    if ($form.length) {
      // Wait for the swatch plugin to update the select
      setTimeout(() => {
        updateForForm($form)
      }, 50)
    }
